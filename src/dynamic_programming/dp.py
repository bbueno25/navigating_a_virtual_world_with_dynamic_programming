...
def one_step_lookahead(environment, state, V, discount_factor):
    ...
    state:
        Agent's state to consider (integer).
    V:
        The value to use as an estimator. Vector of length nS.

    return:
        A vector of length nA containing the expected value
        of each action.
    """
...
def policy_evaluation(policy, environment, discount_factor=1.0, theta=1e-9, max_iter=1e9):
    ...
    discount_factor:
        MDP discount factor. Float in range from 0 to 1.
    environment:
        Initialized OpenAI gym environment object.
    max_iter:
        Maximum number of iteration to prevent infinite loops.
    policy:
        Matrix of a size nSxnA, each cell represents a
        probability of taking action a in state s.
    theta:
        A threshold of a value function change.

    return:
        A vector of size nS, which represent a value function for each state.
    """
    ...
    for _ in range(int(max_iter)):
        ...
        for state in range(environment.nS):
            ...
            for action, action_probability in enumerate(policy[state]):
                ...
                for state_probability, next_state, reward, _ in environment.P[state][action]:
...
def policy_iteration(environment, discount_factor=1.0, max_iter=1e9):
    ...
    discount_factor:
        MPD discount factor. Float in range from 0 to 1.
    environment:
        Initialized OpenAI gym environment object.
    max_iter:
    ...
    return:
        tuple(policy, V), which consist of an optimal policy
        matrix and value function for each state.
    """
    ...
    for _ in range(int(max_iter)):
        ...
        evaluated_policies += 1
        # If the algorithm converged and policy is not changing anymore,
        # than return final policy and value function
        if stable_policy:
...
def value_iteration(environment, discount_factor=1.0, theta=1e-9, max_iterations=1e9):
    ...
    discount_factor:
        MDP discount factor.
    environment:
        Initialized OpenAI environment object.
    max_iterations:
        Maximum number of iterations that can be ever performed
        (to prevent infinite loops).
    theta:
        Stopping threshold. If the value of all states changes
        less than theta in one iteration - we are done.

    return:
        tuple (policy, V) which contains optimal policy
